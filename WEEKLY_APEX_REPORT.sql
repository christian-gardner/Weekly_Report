create or replace procedure WEEKLY_APEX_REPORT
is
...
open c1;
      LOOP
        ...
        Ebody_html := Ebody_html||'<tr> ';
            ...
            Ebody_html := CASE WHEN R1.ACTIVE = 1 THEN Ebody_html||'<td class="right">'||to_char(R1.WORKING,'9,999,999')||'</td>' ELSE Ebody_html||'<td class="right">'||'N/A'||'</td>' END;
...
close c2;
    ...
    EXECUTE IMMEDIATE SQL_STMT USING  'SYS Cancel','Weekly APEX Report','APEX IssueTrak Weekly Usage Report' ,Ebody,Ebody_html;
    commit;
